...
namespace AlbertCollection.Application
{
    ...
    public class RoleService : DbRepository<SysRoleAc>, IRoleService
    {
        ...
        public async Task Add(RoleAddInput input)
        {
            ...
            var sysRole = input.Adapt<SysRoleAc>();//实体转换
        ...
        public async Task Delete(List<BaseIdInput> input)
        {
            ...
            if (ids.Count > 0)
            {
                ...
                var result = await itenant.UseTranAsync(async () =>
                {
                    ...
                    var relationRep = base.ChangeRepository<DbRepository<SysRelationAc>>();//切换仓储
        ...
        public async Task Edit(RoleEditInput input)
        {
            ...
            if (role != null)
            {
                var permissions = new List<SysRelationAc>();
        ...
        /// </summary>
        /// <returns></returns>
        public override async Task<List<SysRoleAc>> GetListAsync()
        {
            //先从Cache拿
            var sysRoles = _sysCacheService.Get<List<SysRoleAc>>(CacheConst.Cache_SysRole, "");
            if (sysRoles == null)
            {
                //cache没有就去数据库拿
        ...
        {
            List<SysRoleAc> cods = new List<SysRoleAc>();//角色代码集合
        ...
        /// <inheritdoc/>
        public async Task<List<SysRoleAc>> GetRoleListByUserId(long userId)
        {
            List<SysRoleAc> cods = new List<SysRoleAc>();//角色代码集合
            var roleList = await _relationService.GetRelationListByObjectIdAndCategory(userId, CateGoryConst.Relation_SYS_USER_HAS_ROLE);//根据用户ID获取角色ID
            var roleIdList = roleList.Select(x => x.TargetId.ToLong()).ToList();//角色ID列表
        ...
        public async Task GrantResource(GrantResourceInput input)
        {
            ...
            var relationRoles = new List<SysRelationAc>();//要添加的角色资源和授权关系表
            ...
            if (sysRole != null)
            {
                ...
                {
                    //将角色资源添加到列表
                    relationRoles.Add(new SysRelationAc
                    {
                        ObjectId = sysRole.Id,
                ...
                var relationRolePer = new List<SysRelationAc>();//要添加的角色有哪些权限列表
                ...
                if (menus.Count > 0)
                {
                    ...
                    {
                        //新建角色权限关系
                        relationRolePer.Add(new SysRelationAc
                        {
                            ObjectId = sysRole.Id,
                ...
                var result = await itenant.UseTranAsync(async () =>
                {
                    var relatioRep = ChangeRepository<DbRepository<SysRelationAc>>();//切换仓储
                                                                                   //删除老的
        ...
        {
            var sysRelations = new List<SysRelationAc>();//关系列表
                                                       //遍历用户ID
            ...
            var result = await itenant.UseTranAsync(async () =>
            {
                var relationRep = ChangeRepository<DbRepository<SysRelationAc>>();//切换仓储
        ...
        /// <inheritdoc/>
        public async Task<SqlSugarPagedList<SysRoleAc>> Page(RolePageInput input)
        {
            var query = Context.Queryable<SysRoleAc>()
        ...
        /// <inheritdoc />
        public async Task<List<SysRoleAc>> RoleSelector(string searchKey = null)
        {
            var result = await Context.Queryable<SysRoleAc>()
        ...
        private async Task CheckInput(SysRoleAc sysRole)
        {
        ...
        /// <returns></returns>
        private async Task<List<SysResourceAc>> GetMenuByMenuIds(List<long> menuIds)
        {
            //获取所有菜单
