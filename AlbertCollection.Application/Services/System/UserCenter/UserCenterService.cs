...
namespace AlbertCollection.Application
{
    ...
    public class UserCenterService : DbRepository<SysUserAc>, IUserCenterService
    {
        ...
        /// <inheritdoc />
        public async Task<List<SysResourceAc>> GetOwnMenu(string UserAccount = null)
        {
            var result = new List<SysResourceAc>();
            //获取用户信息
            ...
            if (userInfo != null)
            {
                ...
                List<SysResourceAc> allMenuList = new List<SysResourceAc>();//菜单列表
                List<SysResourceAc> allSpaList = new List<SysResourceAc>();//单页列表
                //遍历菜单集合
                ...
                //输出的用户权限菜单
                List<SysResourceAc> myMenus = new();
                //管理员拥有全部权限
        ...
        public async Task UpdateUserInfo(UpdateInfoInput input)
        {
            ...
            //更新指定字段
            var result = await UpdateAsync(it => new SysUserAc
            {
        ...
        /// <returns></returns>
        private List<SysResourceAc> GetMyParentMenus(List<SysResourceAc> allMenuList, List<SysResourceAc> myMenus)
        {
            var parentList = new List<SysResourceAc>();
            myMenus.ForEach(it =>
            {
                //找到父ID对应的菜单
