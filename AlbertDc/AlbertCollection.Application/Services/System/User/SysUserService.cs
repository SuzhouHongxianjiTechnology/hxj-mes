...
namespace AlbertCollection.Application
{
    ...
    public class SysUserService : DbRepository<SysUserAc>, ISysUserService
    {
        ...
        public async Task Add(UserAddInput input)
        {
            ...
            var sysUser = input.Adapt<SysUserAc>();//实体转换
        ...
        public void DeleteUserFromCache(List<long> ids)
        {
            ...
            List<SysUserAc> sysUsers = new List<SysUserAc>();
            foreach (var item in userIds)
            {
                var user = _sysCacheService.Get<SysUserAc>(CacheConst.Cache_SysUser, item);//获取用户列表
                sysUsers.Add(user);
        ...
        public async Task DisableUser(BaseIdInput input)
        {
            ...
            if (sysUser != null)
            {
                ...
                CheckSelf(input.Id, AdminConst.Disable);//判断是不是自己
                                                        //设置状态为禁用
                if (await UpdateAsync(it => new SysUserAc { UserStatus = false }, it => it.Id == input.Id))
                {
                    //从列表中删除
        ...
        public async Task EnableUser(BaseIdInput input)
        {
            CheckSelf(input.Id, AdminConst.Enable);//判断是不是自己
                                                   //设置状态为启用
            if (await UpdateAsync(it => new SysUserAc { UserStatus = true }, it => it.Id == input.Id))
                DeleteUserFromCache(input.Id);//从cache删除用户信息
        }
        ...
        /// <inheritdoc/>
        public async Task<SysUserAc> GetUserByAccount(string account)
        {
        ...
        /// <inheritdoc/>
        public async Task<SysUserAc> GetUsertById(long Id)
        {
            //先从Cache拿
            var sysUser = _sysCacheService.Get<SysUserAc>(CacheConst.Cache_SysUser, Id.ToString());
            if (sysUser == null)
            {
                sysUser = await Context.Queryable<SysUserAc>()
                .Where(u => u.Id == Id)
                .Select((u) => new SysUserAc { Id = u.Id.SelectAll() })
                .FirstAsync();
        ...
        /// <inheritdoc/>
        public async Task<SqlSugarPagedList<SysUserAc>> Page(UserPageInput input)
        {
            var query = Context.Queryable<SysUserAc>()
        ...
        public async Task ResetPassword(BaseIdInput input)
        {
            ...
            //重置密码
            if (await UpdateAsync(it => new SysUserAc { Password = password }, it => it.Id == input.Id))
            {
                //从列表中删除
                _sysCacheService.SetVerificatId(input.Id, new());
                DeleteUserFromCache(input.Id);//从cache删除用户信息
        ...
        {
            var result = await Context.Queryable<SysUserAc>()
        ...
        private async Task CheckInput(SysUserAc sysUser)
        {
            //判断账号重复,直接从cache拿
