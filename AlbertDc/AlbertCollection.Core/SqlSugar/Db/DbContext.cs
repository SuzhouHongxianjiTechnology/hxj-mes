        ...
        public static void AopSetting(SqlSugarScopeProvider db)
        {
            ...
            db.Aop.DataExecuting = (oldValue, entityInfo) =>
            {
                ...
                if (entityInfo.OperationType == DataFilterType.InsertByObject)
                {
                    ...
                    //手机号和密码自动加密
                    if (entityInfo.PropertyName == nameof(SysUserAc.Password) || entityInfo.PropertyName == nameof(SysUserAc.Phone))
            ...
            {
                if (entity.Entity.Type == typeof(SysUserAc) || entity.Entity.Type == typeof(OpenApiUser))
                {
                    //如果手机号不为空
                    if (entity.GetValue(nameof(SysUserAc.Phone)) != null)
                    {
                        //手机号数据转换
                        var phone = CryptogramUtil.Sm4Decrypt(entity.GetValue(nameof(SysUserAc.Phone)).ToString());
                        entity.SetValue(nameof(SysUserAc.Phone), phone);
                    }
                    //如果密码不为空
                    if (entity.GetValue(nameof(SysUserAc.Password)) != null)
                    {
                        //密码数据转换
                        var passwd = CryptogramUtil.Sm4Decrypt(entity.GetValue(nameof(SysUserAc.Password)).ToString());
                        entity.SetValue(nameof(SysUserAc.Password), passwd);
                    }
                }
            };
        }
